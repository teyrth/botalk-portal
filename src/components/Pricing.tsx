      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <div className="animate-on-scroll opacity-0 animation-delay-500 flex items-center justify-center mt-8">
            <div className="bg-white border border-gray-200 p-1 rounded-full inline-flex">
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={cn(
                "animate-on-scroll opacity-0 bg-white rounded-xl overflow-hidden border transition-all",
                plan.popular
                  ? "border-chatbot-primary shadow-strong relative md:-mt-4 md:mb-4 scale-[1.02]"
                  : "border-gray-200 shadow-card",
                index === 0 ? "animation-delay-200" : index === 1 ? "animation-delay-500" : "animation-delay-700"
              )}
            >
              {plan.popular && (
                <div className="bg-chatbot-primary text-white text-xs font-bold uppercase tracking-wider py-1.5 text-center">
                  Most Popular
                </div>
              )}
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-6 mb-6">
                  <div className="flex items-baseline">
                    ...
                    {typeof plan.price[billingCycle] === 'number' && plan.price[billingCycle] > 0 && (
                      <span className="text-gray-500 ml-2">/{billingCycle === 'monthly' ? 'mo' : 'mo, billed annually'}</span>
                    )}
                  </div>
                </div>
                ...
                <Button
                  className={cn(
                    "w-full",
                    plan.popular
                      ? "bg-chatbot-primary hover:bg-chatbot-secondary text-white"
                      : "bg-white text-chatbot-primary border border-chatbot-primary hover:bg-chatbot-light"
                  )}
                >
                  {plan.cta}
                </Button>
        ...
        {/* Enterprise CTA */}
        <div className="animate-on-scroll opacity-0 animation-delay-700 mt-16 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-8 md:p-12 text-white shadow-lg">
